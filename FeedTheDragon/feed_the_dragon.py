import pygame
import random

# Initalize pygame
pygame.init()

# Set display surface
WINDOW_WIDTH, WINDOW_HEIGHT = 1000, 400
window = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
pygame.display.set_caption("Feed the Dragon")

# set fps and clock
FPS = 60
clock = pygame.time.Clock()

# set game values
PLAYER_STARTING_LIVES = 5
PLAYER_VELOCITY = 5
COIN_STARTING_VELOCITY = 5
COIN_ACCELERATION = .5
BUFFER_DISTANCE = 100

score = 0
player_lives = PLAYER_STARTING_LIVES
coin_velocity = COIN_STARTING_VELOCITY

# set colors
GREEN = (0, 255, 0)
DARKGREEN = (10, 50, 10)
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)

# set fonts
font = pygame.font.Font('assets/images/AttackGraffiti.ttf', 32)

# set texts
score_text = font.render(f'Score: {str(score)}', True, GREEN, DARKGREEN)
score_rect = score_text.get_rect()
score_rect.topleft = (10, 10)

title_text = font.render('Feed the Dragon', True, GREEN, WHITE)
title_rect = title_text.get_rect()
title_rect.centerx = WINDOW_WIDTH // 2
title_rect.y = 10

lives_text = font.render(f'Lives: {str(player_lives)}', True, GREEN, DARKGREEN)
lives_rect = lives_text.get_rect()
lives_rect.topright = (WINDOW_WIDTH - 10, 10)

game_over_text = font.render('GAMEOVER', True, GREEN, DARKGREEN)
game_over_rect = game_over_text.get_rect()
game_over_rect.center = (WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2)

continue_text = font.render('Press any key to play again', True, GREEN, DARKGREEN)
continue_rect = continue_text.get_rect()
continue_rect.center = (WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 32)


# set sounds and music
coin_sound = pygame.mixer.Sound("assets/images/coin_sound.wav")
miss_sound = pygame.mixer.Sound("assets/images/miss_sound.wav")
miss_sound.set_volume(.1)
pygame.mixer_music.load('assets/images/ftd_background_music.wav')

# set images
player_image = pygame.image.load('assets/images/dragon_right.png')
player_rect = player_image.get_rect()
player_rect.left = 32
player_rect.centery = WINDOW_HEIGHT // 2

coin_image = pygame.image.load('assets/images/coin.png')
coin_rect = coin_image.get_rect()
coin_rect.x = WINDOW_WIDTH + BUFFER_DISTANCE
coin_rect.y = random.randint(64, WINDOW_HEIGHT - 32)

# pause the game until player presses a key, then reset the game
def pause_game():
    pygame.mixer.music.stop()
    is_paused = True
    while is_paused:
        for event in pygame.event.get():
            # The player wants to play again
            if event.type == pygame.KEYDOWN:
                score = 0
                player_lives = PLAYER_STARTING_LIVES
                player_rect.y = WINDOW_HEIGHT // 2
                pygame.mixer_music.play(-1, 0, 0)
                # is_paused = False
                return score, player_lives, COIN_STARTING_VELOCITY, True
            
            # if Player wants to quit
            if event.type == pygame.QUIT:
                # is_paused = False
                return 0, 0, False
    
    

# The main game loop
pygame.mixer.music.play(-1, 0, 0)
running = True
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
    
    # Check to see if the user wants to move
    keys = pygame.key.get_pressed()
    
    if keys[pygame.K_w] and player_rect.top > 64:
        player_rect.y -= PLAYER_VELOCITY
    
    if keys[pygame.K_s] and player_rect.bottom < WINDOW_HEIGHT:
        player_rect.y += PLAYER_VELOCITY

    if player_lives == 0:
        window.blit(game_over_text, game_over_rect)
        window.blit(continue_text, continue_rect)
        pygame.display.update()
        
        # Pause the game until player presses a key, then we will reset the game
        score, player_lives, coin_velocity ,running = pause_game()
        print(coin_velocity)

        # reset the hud text
        lives_text = font.render(f'Lives: {str(player_lives)}', True, GREEN, DARKGREEN)
        score_text = font.render(f'Score: {str(score)}', True, GREEN, DARKGREEN)
        pygame.display.update()
    
    # Move the coin
    if coin_rect.x < 0:
        # player miss the coin
        player_lives -= 1
        miss_sound.play()
        coin_rect.x = WINDOW_WIDTH + BUFFER_DISTANCE
        coin_rect.y = random.randint(64, WINDOW_HEIGHT - 32)
        # Update life
        lives_text = font.render(f'Lives: {str(player_lives)}', True, GREEN, DARKGREEN)
    
        
    
    else:
        # Move the coin
        coin_rect.x -= coin_velocity
        
    
    # Check for collision 
    if player_rect.colliderect(coin_rect):
        # when player collides with coin
        # increase score, and coin speed
        score += 1
        coin_sound.play()
        coin_velocity += COIN_ACCELERATION
        coin_rect.x = WINDOW_WIDTH + BUFFER_DISTANCE
        coin_rect.y = random.randint(64, WINDOW_HEIGHT - 32)

        # Update score
        score_text = font.render(f'Score: {str(score)}', True, GREEN, DARKGREEN)
    
    

    # fill the display
    window.fill(BLACK)

    # Blit the Hud to screen
    window.blit(score_text, score_rect)
    window.blit(title_text, title_rect)
    window.blit(lives_text, lives_rect)
    pygame.draw.line(window, WHITE, (0,64), (WINDOW_WIDTH, 64), 2)

    # Blit player to screen
    window.blit(player_image, player_rect)
    window.blit(coin_image, coin_rect)

    # update screen
    pygame.display.update()

    # tick the clock
    clock.tick(FPS)

# End the game
pygame.quit()